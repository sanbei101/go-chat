      ...
      <section class="grid flex-1 gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
        <Card class="flex min-h-[720px] flex-col overflow-hidden bg-background/88 backdrop-blur">
          <div class="space-y-4 p-4">
            ...
            <p v-if="roomError" class="text-sm text-destructive">{{ roomError }}</p>
          </div>
            ...
            >
              <div class="flex items-center justify-between gap-3">
                <div class="min-w-0">
                  ...
                  <p class="text-xs text-muted-foreground">room #{{ room.id }}</p>
                </div>
                <Badge variant="outline">在线 {{ room.id === activeRoomId ? members.length : "-" }}</Badge>
              </div>
            </button>

            <div v-if="!rooms.length && !roomsLoading" class="rounded-xl border border-dashed px-4 py-8 text-center text-sm text-muted-foreground">
              还没有聊天室
            </div>
          </div>
        </Card>
          ...
          <div class="flex items-center justify-between gap-3 p-4">
            <div>
              ...
              <p class="text-sm text-muted-foreground">
                {{ activeRoom ? `房间 ID ${activeRoom.id}` : "加入后即可开始消息收发" }}
              </p>
            </div>
            <div class="flex items-center gap-2 rounded-full bg-secondary px-3 py-1.5 text-sm text-secondary-foreground">
              <Users class="size-4" />
              ...
              <div ref="messageListRef" class="flex-1 space-y-3 overflow-y-auto p-4">
                <template v-if="messages.length">
                  <div
                    ...
                    class="flex"
                    :class="message.user_id === auth?.userId ? 'justify-end' : 'justify-start'"
                  >
                    <div
              ...
              <div class="p-4">
                <form class="space-y-3" @submit.prevent="handleSendMessage">
                  ...
                  <div class="flex items-center justify-between gap-3">
                    ...
                    <Button type="submit" :disabled="!activeRoomId || !draft.trim() || sending">
                      <Send class="size-4" />
                      发送
                    </Button>
                  </div>
            ...
            <div class="border-t p-4 lg:border-t-0 lg:border-l">
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <Badge :variant="member.id === auth?.userId ? 'default' : 'outline'">
                    {{ member.id === auth?.userId ? "你" : "在线" }}
                  </Badge>
                </div>

                <div v-if="!members.length" class="rounded-xl border border-dashed px-3 py-6 text-center text-sm text-muted-foreground">
                  暂无在线成员
                </div>
              </div>
            </div>
