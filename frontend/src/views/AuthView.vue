...
<template>
  <main
    class="min-h-screen bg-linear-to-b from-white to-stone-50 px-4 py-6 text-foreground md:px-6"
  >
    <div class="mx-auto flex min-h-[calc(100vh-3rem)] max-w-7xl flex-col gap-4">
      <section
        class="flex items-center rounded-2xl border bg-background/80 px-4 py-3 shadow-sm backdrop-blur md:px-5"
      >
      ...
      <section class="grid flex-1 place-items-center">
        <Card
          class="w-full max-w-md border-0 bg-background/90 p-6 shadow-lg shadow-black/5"
        >
          ...
          <form class="space-y-3" @submit.prevent="handleSubmit">
            ...
            <Input
              ...
              type="password"
            />
            <p v-if="authError" class="text-sm text-destructive">
              {{ authError }}
            </p>
